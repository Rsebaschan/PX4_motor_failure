...
MavlinkInterface::MavlinkInterface() : serial_dev_(io_service_)
{
}
...
void MavlinkInterface::SendHeartbeat()
{
  ...
  if (hil_mode_)
  {
    // mavlink_message_t는 이러한 구조를 가진다
...
void MavlinkInterface::SendSensorMessages(uint64_t time_usec, HILData &hil_data)
{
  ...
  if (!hil_mode_ || (hil_mode_ && !hil_state_level_))
  {
    ...
    // sensor_msg 에는 imu,mag, baro, diff 의 센서 데이터가 담겨있다.
...
{
  // const std::lock_guard<std::mutex> lock(actuator_mutex_); 는 actuator_mutex_를 잠금으로써,
  // 이 코드 블록 내에서 실행되는 작업이 여러 스레드에 의해 동시에 실행되지 않도록 보호합니다.
  // 이는 멀티스레드 프로그래밍에서 안전하게 공유 자원에 접근하기 위해 중요한 역할을 합니다.
  // 내 생각은 actuator_mutex_가 여러 함수에서 작동 안되게할려고 하는듯? 하나의 함수에서 이뤄지고 다름 함수 이뤄지고 그런느낌으로 작동하기 위해 적은듯
  ...
  // MAV_MODE_FLAG_SAFETY_ARMED는 MAVLink 프로토콜에서 드론이나 다른 무인 항공기 시스템이 "ARMED"(무장됨)상태인지 여부를 나타내는 플래그입니다.
  // 이미지에 나와 있는 비트 값인 0b10000000은 이 플래그가 128이라는 값에 해당한다는 것을 나타냅니다. 비트 마스크를 사용해 특정 상태를 나타내기 때문에, 이 값이 설정되면 드론이 "ARMED" 상태에 있음을 알 수 있습니다.

  // 이 코드는 비트 연산자를 이용하여 계산한다.
  ...
  // std::cout << "Control values:" << std::endl;
  for (int i = 0; i < input_reference_.size(); i++)
  {
    input_reference_[i] = controls.controls[i];
    // std::cout << "Control[" << i << "]: " << controls.controls[i] << std::endl;
...
{
  // handle_actuator_controls 함수에서 input_reference_ 값이 계산되어진다.
  const std::lock_guard<std::mutex> lock(actuator_mutex_);
