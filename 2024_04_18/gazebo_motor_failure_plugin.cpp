...
namespace gazebo
{
  ...
  GazeboMotorFailure::GazeboMotorFailure() : ModelPlugin(),
  ...
  void GazeboMotorFailure::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    */
    ...
    node_handle_->Init(namespace_); ////Gazebo 통신을 위한 노드 핸들
    ...
    { // sdf 파일에 지금 이 줄이 없다 ROSMotorNumSubTopic ******************************
    ...
    if (_sdf->HasElement("ROSMotorNumSubTopic1"))
    { // sdf 파일에 지금 이 줄이 없다 ROSMotorNumSubTopic ******************************
    ...
    if (_sdf->HasElement("ROSMotorNumSubTopic2"))
    { // sdf 파일에 지금 이 줄이 없다 ROSMotorNumSubTopic ******************************
    ...
    if (_sdf->HasElement("ROSMotorNumSubTopic3"))
    { // sdf 파일에 지금 이 줄이 없다 ROSMotorNumSubTopic ******************************
    ...
    { // sdf 파일에 지금 이 줄이 없다 MotorFailureNumPubTopic ******************************
